...
`kubectl run web1 --image=nginx --dry-run -o yaml` <br>
`kubectl run web1 --image=nginx --dry-run=client -o yaml` <br>
`kubectl run web1 --image=nginx --dry-run=client -o yaml > nginx.yaml` <br>
...
```bash
kubectl describe pod pod1
kubectl exec -it pods/<podname> -c <containername> -- <command>
```
