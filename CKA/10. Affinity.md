...
#### Example: `db` Pod ad and `web` Pod will run on same node (podAffinity)
...
```
We can see both pods are run on same node
```bash
kubectl get pods -o wide
```

#### Example: `db` Pod ad and `web` Pod will run on different node. (podAntiAffinity)
```bash
kubectl run dbpod --image nginx --labels pod=db
```
```yaml
...
kubectl apply -f web.yaml
```
We can see db pod and web pod will run on different node
```bash
kubectl get pods -o wide
```

#### Example: Pod will run different zone
- worker1 = banani zone
- worker2 = banani zone
- worker3 = dhanmondi zone
...
We will see db pod and web pod run in different zone
```bash
kubectl get pods -o wide
```


#### Example: Pod will run same zone
```bash
kubectl run dbpod --image nginx --labels pod=db
```
...
We will see the db pod and web pod will run in same zone
```bash
kubectl get pods -o wide
```
...
### taint
effect jodi noSchedule dei, new taint jokhon bosbe tokhon theke new pod eta jodi tolratare na kore amar ekhane schedule hobe na but exixting gula thakbe 
```bash
...
#### untaint
This command will be noSchedule pod and previous pod will run as well 
NoSchedule  
```bash
kubectl cordon worker1 worker2 worker3
kubectl get nodes
...
New POD ei value jodi tolarate na kore taile schedule hobe na and exixting pod er sathe tolatation taint er match na kore, tara o ekhan theke ber hoye jabe 
```bash
kubectl create deployment dept1 --image nginx --replicas 5
kubectl taint node master1 node-role.kubernetes.io/control-plane:NoExecute
kubectl get pods

#Now all POD will be running
kubectl taint node master1 node-role.kubernetes.io/control-plane:NoExecute-
```


```bash
kubectl describe nodes master1 | grep -i taint -C 5
kubectl taint node master1 node-role.kubernetes.io/control-plane:PreferNoSchedule
kubectl run pod3 --image nginx
```


### Node e jodi Tain set kora thake NoSchedule, tahole POD theke tolarate korbo 
```bash
kubectl taint node master1 node-role.kubernetes.io/control-plane:NoSchedule
...
kubectl apply -f pod1.yaml # pod in pending state
kubectl delete pod --all
```
...
kubectl describe nodes | grep -i taint -C 5
```
